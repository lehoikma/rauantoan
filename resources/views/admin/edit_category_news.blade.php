...
@section('content')
    <div class="col-md-12">
        ...
        <form method="post" action="{{route('edit_category_news')}}">
            {{csrf_field()}}
            <div class="col-md-8">
                <div class="form-group">
                    <input type="hidden" name="id_category_news" value="{{$categoryNews['id']}}">
                    <input type="text" name="name_category_news" class="form-control" value="{{$categoryNews['name']}}">
                </div>
            </div>
            <div class="col-md-4">
    ...
    <div class="col-md-12" >
        <div class="col-md-12" ><h4>Danh Sách Danh Mục</h4>
            <table class="table table-striped">
                ...
                <tbody>
                @if(!empty($categoryNewsList))
                    @foreach($categoryNewsList as $value)
                        <tr>
                            <td>{{$value['id']}}</td>
                            <td>{{$value['name']}}</td>

...
@section('content')
    <div class="col-md-12">
        ...
        <form method="post" action="{{route('edit_category_product')}}">
            {{csrf_field()}}
            <div class="col-md-8">
                <div class="form-group">
                    <input type="hidden" name="id_category_prd" value="{{$categoryProduct['id']}}">
                    <input type="text" name="name_category_prd" class="form-control" value="{{$categoryProduct['name']}}">
                </div>
            </div>
            <div class="col-md-4">
                ...
                @if(!empty($categoryProducts))
                    @foreach($categoryProducts as $value)
                        <tr>
                            ...
                            <td>
                                <a href="{{route('show_edit_category_product',$value['id'])}}">
                                    <button class="btn btn-warning btn-sm" data-id="{{$value['id']}}"><i class="fa fa-trash"></i> Sửa</button>
                                </a>
                                <a href="{{route('delete_category_product',$value['id'])}}">
